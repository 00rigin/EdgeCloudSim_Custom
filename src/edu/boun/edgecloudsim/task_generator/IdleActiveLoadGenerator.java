...
public class IdleActiveLoadGenerator extends LoadGeneratorModel{
	...
	public void initializeModel() {
		...
		// [13] task deadline (sec) 20211015 HJ �߰�
		...
		for(int i=0; i<SimSettings.getInstance().getTaskLookUpTable().length; i++) {
			...
			expRngList[i][3] = new ExponentialDistribution(SimSettings.getInstance().getTaskLookUpTable()[i][13]); //20211015 HJ �߰� deadline property
		...
		for(int i=0; i<numberOfMobileDevices; i++) {
			...
			double length = SimSettings.getInstance().getTaskLookUpTable()[randomTaskType][7];
			double deadline = SimSettings.getInstance().getTaskLookUpTable()[randomTaskType][13];
