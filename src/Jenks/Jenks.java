...
/*
 * How to use?
 double[] list =	new double[tasks.size()];
		for(int i = 0; i<list.length;i++) {
			list[i] = tasks.get(i).doubleValue();
		}
Jenks jen = new Jenks();
jen.addValues(list);
System.out.print(jen.computeBreaks(3));	
 * 
 * 
 * 
 * 
...
public class Jenks {
    ...
    List<Integer> classList = new ArrayList<Integer>(); // for class return
    ...
    public  Breaks computeBreaks(double[] list, int numclass) {
    ...
    public static class Breaks {
        ...
        public double getDivider(int _index) { //Retrun Each class's maximum
        	return sortedValues[breaks[_index]];
        }
